...
const (
	retries  = 2
	waitTime = 1 * time.Second // Constant 1 seconds delay between retries
)
...
// the 3rd attempt will log an error. The retry wait time is fixed at 1 seconds.
	...
	for path := range fileDeleteChan {
		...
		for i := 0; i <= retries; i++ {
			...
			if i < retries && strings.Contains(err.Error(), "The process cannot access the file because it is being used by another process.") {
				log.Warn().Msgf("Path: %s, Attempt %d to delete file failed because it is in use. Retrying in 1 seconds...", path, i+1)
